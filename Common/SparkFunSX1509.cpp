...
//TODO: fix code to properly switch from analog to digital mode

//TODO: this function will probably fail if the lower limit is less than zero. Fix this later...
...
int SX1509_fd;	//TODO: Make this part of the class, otherwise it won't work with multiple expanders.
...
SX1509::SX1509(unsigned char address, unsigned char resetPin, unsigned char interruptPin, unsigned char oscillatorPin)
{
...
unsigned char SX1509::init(void)
{
	...
	//Initialize I2C
	SX1509_fd = wiringPiI2CSetup(deviceAddress);
...
void SX1509::reset(bool hardware)
{
	...
	if (hardware)
	{
		...
		//TODO: This appears to implement a hardware reset from the MCU. Probably don't want this here...
...
void SX1509::sync(void)
{
	...
	pinMode(pinReset, OUTPUT);	// set reset pin as output
	digitalWrite(pinReset, LOW);	// pull reset pin low
	//delay(1);	// Wait for the pin to settle					//TODO: This function is an Arduino fuinction, figure out some other way to do this...
	digitalWrite(pinReset, HIGH);	// pull reset pin back high	
...
unsigned char SX1509::calculateLEDTRegister(int ms)
{
	...
	regOn2 = regOn1 / 8;
	regOn1 = constrain(regOn1, 1, 15);		//TODO: fix this to work with int...
	regOn2 = constrain(regOn2, 16, 31);		//TODO: fix this to work with int...
...
unsigned char SX1509::calculateSlopeRegister(int ms, unsigned char onIntensity, unsigned char offIntensity)
{
	...
	regSlope1 = constrain(regSlope1, 1, 15);	//TODO: fix this to work with int...
	regSlope2 = constrain(regSlope2, 16, 31);	//TODO: fix this to work with int...
...
unsigned char SX1509::readByte(unsigned char registerAddress)
{
	...
	{
		//Failed to initialize I2c
		//TODO: Do I add initialization code here?
		return 0;
	}

	result = wiringPiI2CReadReg8(SX1509_fd, registerAddress);
...
unsigned int SX1509::readWord(unsigned char registerAddress)
{
	...
	//There is an endianness issue with wiringPiI2CReadReg16(), use two 8-bit transfers instead.
	result = wiringPiI2CReadReg8(SX1509_fd, registerAddress);
	result = result << 8;
	result |= wiringPiI2CReadReg8(SX1509_fd, registerAddress+1);
	
	return (unsigned int)result;
}

// readBytes(unsigned char firstRegisterAddress, unsigned char * destination, unsigned char length)
...
//	- No return value.
void SX1509::readBytes(unsigned char firstRegisterAddress, unsigned char * destination, unsigned char length)
{
	int result;
	unsigned char RegToRead;
	
	RegToRead = firstRegisterAddress;
	...
	{
		destination[i] = wiringPiI2CReadReg8(SX1509_fd, RegToRead);
		RegToRead++;
	}
}
...
void SX1509::writeByte(unsigned char registerAddress, unsigned char writeValue)
{
	...
	wiringPiI2CWriteReg8(SX1509_fd, registerAddress, writeValue);
}
...
void SX1509::writeWord(unsigned char registerAddress, unsigned int writeValue)
{
	...
	//There is an endianness issue with wiringPiI2CWriteReg16(), use two 8-bit transfers instead.
	wiringPiI2CWriteReg8(SX1509_fd, registerAddress, ((writeValue>>8)&(0xFF)) );
	wiringPiI2CWriteReg8(SX1509_fd, registerAddress+1, (writeValue&0xFF));
}

// writeBytes(unsigned char firstRegisterAddress, unsigned char * writeArray, unsigned char length)
...
//	- no return value.
void SX1509::writeBytes(unsigned char firstRegisterAddress, unsigned char * writeArray, unsigned char length)
{
	int result;
	unsigned char RegToWrite;

	RegToWrite = firstRegisterAddress;
	...
	for (int i=0; i<length; i++)
	{
		wiringPiI2CWriteReg8(SX1509_fd, RegToWrite, writeArray[i]);
		RegToWrite++;
	}
}