...
void Adafruit_LEDBackpack::setBrightness(uint8_t b) {
  ...
  wiringPiI2CWrite(LEDBackpack_fd, (HT16K33_CMD_BRIGHTNESS | b) );
...
void Adafruit_LEDBackpack::blinkRate(uint8_t b) {
  ...
  wiringPiI2CWrite(LEDBackpack_fd, (HT16K33_BLINK_CMD | HT16K33_BLINK_DISPLAYON | (b << 1)) );
...
void Adafruit_LEDBackpack::begin(uint8_t _addr = 0x70) 
{
	...
	LEDBackpack_fd = wiringPiI2CSetup(i2c_addr);
	...
	wiringPiI2CWrite(LEDBackpack_fd, 0x21);		//This command turns on the oscillator
...
void Adafruit_LEDBackpack::writeDisplay(void)
{
	...
	{
		wiringPiI2CWriteReg8 (LEDBackpack_fd, (i*2), (displaybuffer[i] & 0xFF) );
		wiringPiI2CWriteReg8 (LEDBackpack_fd, (i*2)+1, (displaybuffer[i] >> 8) );
	}
...
void Adafruit_AlphaNum4::writeDigitAscii(uint8_t n, uint8_t a,  bool d) {
  //uint16_t font = pgm_read_word(alphafonttable+a);
	...
	uint16_t font = alphafonttable[a];	//TODO: I am assuming that a starts at zero, make sure this is right...
...
void Adafruit_7segment::writeColon(void) 
{
	...
	wiringPiI2CWrite(LEDBackpack_fd, 0x04);
	wiringPiI2CWrite(LEDBackpack_fd, (displaybuffer[2] & 0xFF) );
	wiringPiI2CWrite(LEDBackpack_fd, (displaybuffer[2] >> 8) );
